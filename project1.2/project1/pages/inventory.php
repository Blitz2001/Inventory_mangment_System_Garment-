...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['adjust_stock'])) {
    ...
    try {
        ...
        // Update inventory - ensure record exists first
        $stmt = $conn->prepare("INSERT INTO inventory (product_id, quantity_on_hand) 
                               VALUES (?, ?) 
                               ON DUPLICATE KEY UPDATE quantity_on_hand = quantity_on_hand + VALUES(quantity_on_hand)");
        $stmt->bind_param("ii", $product_id, $adjustment);
        $stmt->execute();
        $stmt->close();
    ...
    <tbody>
        <?php if ($result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): 
                ...
                $product_status = $row['is_active'] ? 'active' : 'inactive';
                $product_status_text = $row['is_active'] ? 'Active' : 'Inactive';
            ?>
                <tr>
                    ...
                    <td>
                        <span class="status <?php echo $product_status; ?>">
                            <?php echo $product_status_text; ?>
                        </span>
                        <br>
